...
<div class="content">
    ...
    <div class="container-fluid">
                ...
                <div class="page-title-box">
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active">ประเภทงบ</li>
                        </ol>
                    </div>
                    <h4 class="page-title">แก้ไข ประเภทงบ</h4>
                </div>
            </div>
        </div>
        <!-- end page title -->
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card-box">
                    <h4 class="header-title mb-4"><i class="mdi mdi-file-document-box-plus" style="font-size: 16px !important; "> แก้ไขข้อมูล ประเภทงบ</i> </h4>

                    <form action="{{url('office/settings/data/update')}}" method="POST" name="frm-save" id="frm-save">
                        <input type="hidden" name="action" value="edit">
                        <input type="hidden" name="edit_id" value="{{$id}}}">
                        <input type="hidden" name="input[parent_id_b]" id="parent_id_b" value="{{$pr}}">
                        <input type="hidden" name="input[group_type]" id="group_type" value="{{$t}}">

                        <div class="row">
                            <?php 
                                $statementtypes = \App\Models\DataSetting::where('group_type','statementtype')->where('is_deleted', '0')->where('is_active','1')->get();
                            ?>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="exampleInputEmail1">ประเภทงบหลัก <code>*</code></label>
                                    <select name="input[parent_id]" class="form-control" id="input-parent_id" style="height: 45px;">
                                        <option value="0">--เลือก--</option>
                                        @if (!empty($statementtypes))
                                        @foreach ($statementtypes as $statementtype)
                                            <option value="{{$statementtype['id']}}" @if($statementtype['id'] == $info->parent_id) selected @endif>{{$statementtype['name']}}</option>
                                        @endforeach
                                        @endif
                                    </select>
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="exampleInputEmail1">ประเภทงบ <code>*</code></label>
                                    <input type="text" name="input[name]" class="form-control" id="input-name" placeholder="ประเภทงบ" style="height: 45px;" value="{{$info->name}}">

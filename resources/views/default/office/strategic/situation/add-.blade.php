...
<div class="content">
    ...
    <div class="container-fluid">
        ...
        <form action="{{url('office/strategic/oil/save')}}" method="POST" name="frm-save" id="frm-save">         
            ...
            <?php 
                $oilTypeCount = \App\Models\DataSetting::where('group_type', "situation")->where('parent_id', $oilGroups['id'])->where('is_deleted', '0')->where('is_active','1')->count();
                $oilType = \App\Models\DataSetting::where('group_type', "situation")->where('parent_id', $oilGroups['id'])->where('is_deleted', '0')->where('is_active','1')->get(); 
            ?>
            <?php $no++;?>
                            ...
                            <i class="mdi mdi-file-document-box-plus" style="font-size: 16px !important; "> รายละเอียด : {{$oilGroups['name']}}</i> </h4>
                             <?php $no = 0;?>
                                ...
                                <thead>
                                    ...
                                    <input type="hidden" name="oil_min[{{$oilGroups['id']}}][]" class="form-control" placeholder="" style="height: 45px;">
                                    <input type="hidden" name="oil_max[{{$oilGroups['id']}}][]" class="form-control" placeholder="" style="height: 45px;">

                                    @foreach ($oilType as $item)
                                    <?php
                                        $oilTypeCount2 = \App\Models\DataSetting::where('group_type', "situation")->where('parent_id', $item['id'])->where('is_deleted', '0')->where('is_active','1')->count();
                                        $oilType2 = \App\Models\DataSetting::where('group_type', "situation")->where('parent_id', $item['id'])->where('is_deleted', '0')->where('is_active','1')->get();
                                    ?>
                                    <?php $no1++;?>
                                        @if ($oilTypeCount2 > 0)
                                            <tbody>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="align-middle" align="right">
                                                <input type="text" name="oil_min[{{$oilGroups['id']}}][]" class="form-control" placeholder="" style="height: 45px;">
                                                <input type="hidden" name="oil_max[{{$oilGroups['id']}}][]" class="form-control" placeholder="" style="height: 45px;">
                                            </td>
                                        </tr> 
                                    </tbody>

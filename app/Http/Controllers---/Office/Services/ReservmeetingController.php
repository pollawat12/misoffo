...
/**
 * ReservmeetingController
 */
class ReservmeetingController extends Base 
{    
    /**
     * index
     *
    ...
    {
        $meetings = ReserveMeeting::getMeetingLists();
        
        # code...
        $data = ['meetings'];
        return view('default.office.service.reserve-meeting.index', compact($data))->render();
    }
    ...
    {
        $conditions = ['group_type' => 'meeting_room', 'is_deleted' => (int) 0, 'is_active' => (int) 1];
        $rooms = DataSetting::where($conditions)->get();

        $conditions = ['group_type' => 'meeting_type', 'is_deleted' => (int) 0, 'is_active' => (int) 1];
        ...
        return view('default.office.service.reserve-meeting.add', compact($data))->render();
    }
    ...
    public function edit($id, Request $request)
    {
        ...
        $info = ReserveMeeting::find((int) $id);
        // accessory items
        ...
        return view('default.office.service.reserve-meeting.edit', compact($data))->render();
    }
    ...
    public function save(Request $request)
    {
        ...
        if ($request->ajax() && $request->isMethod('POST')) {
            ...
            $existsMeeting = ReserveMeeting::getMeetingExists($date_start_checked, $date_end_checked, $input['meeting_room_id']);
            ...
            $result = ReserveMeeting::insertArray($array);
    ...
    public function update(Request $request)
    {
        ...
        if ($request->ajax() && $request->isMethod('POST')) {
            ...
            $date_start_checked = getDateTimeForCheck($date_start);
            $date_end_checked = getDateTimeForCheck($date_end, 'end');
            $existsMeeting = ReserveMeeting::getMeetingExistsUpdate($date_start_checked, $date_end_checked, $input['meeting_room_id'], $edit_id);
            // $existsMeeting = ReserveMeeting::getMeetingExistsUpdate($date_start_checked, $date_end_checked, $input['meeting_room_id'], );
            if (count($existsMeeting) > 0) {
                $response = ['status' => false, 'msg' => 'ไม่สามารถจองห้องประชุมนี้ได้ เนื่องจากมีผู้จองแล้ว!'];
                return response()->json($response, 200);
            }
            ...
            $result = ReserveMeeting::updateArray($array);
    ...
    public function delete(Request $request)
    {
        ...
        if ($request->ajax() && $request->isMethod('GET')) {
            $id = $request->del_id;
            $result = ReserveMeeting::deleteRow($id);

    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <h4 class="page-title">แสดง ปริมาณการใช้น้ำมันเชื้อเพลิง</h4>
                </div>
            </div>
        </div>
        <!-- end page title -->
        ...
        <form action="{{url('office/strategic/oil/save')}}" method="POST" name="frm-save" id="frm-save">        
            ...
            <div class="row" >
                <div class="col-md-12">
                    <div class="card-box">
                    <h4 class="header-title mb-4"><i class="mdi mdi-file-document-box-plus" style="font-size: 16px !important; "> เพิ่มข้อมูล ปริมาณการใช้น้ำมันเชื้อเพลิง</i> </h4>
                                ...
                                <table id="datatable" class="table table-bordered" style="width: 300%">
                                    ...
                                    <thead>
                                        <tr class="bg-dark text-white">
                                            <th rowspan="3" style="width: 0.5%!important;" class="align-middle">Date</th>
                                            @if (!empty($oilGroup))
                                            @foreach ($oilGroup as $oilGroups)
                                            <?php 
                                                $oilTypes = \App\Models\DataSetting::where('group_type', "oilGroup")->where('parent_id', $oilGroups['id'])->where('is_deleted', '0')->where('is_active','1')->get(); 
                                                $num = 0;
                                                foreach ($oilTypes as $oilType){

                                                    $oilTypeCount = \App\Models\DataSetting::where('group_type', "oilGroup")->where('parent_id', $oilType['id'])->where('is_deleted', '0')->where('is_active','1')->count();
                                            ...
                                            ?>
                                                <th colspan="{{$num}}" style="width: 2%" class="align-middle text-center">{{ $oilGroups['name'] }}</th>
                                            @endforeach
                                            @endif
                                        </tr>
                                        <tr class="bg-dark text-white">
                                            ...
                                            <?php 
                                                $oilTypes1 = \App\Models\DataSetting::where('group_type', "oilGroup")->where('parent_id', $oilGroups1['id'])->where('is_deleted', '0')->where('is_active','1')->get(); 
                                                if(count($oilTypes1) > 0){
                                            ?>
                                                @foreach ($oilTypes1 as $oilType1)
                                                <?php
                                                     $oilTypeCount1 = \App\Models\DataSetting::where('group_type', "oilGroup")->where('parent_id', $oilType1['id'])->where('is_deleted', '0')->where('is_active','1')->count();
                                                ?>
                                                    <th colspan="{{$oilTypeCount1}}" style="width: 1%" class="align-middle text-center">{{ $oilType1['name'] }}</th>
                                                @endforeach
                                            <?php
                                                }else{
                                            ?>
                                                    <th rowspan="2" style="width: 1%" class="align-middle text-center"></th>
                                            <?php
                                                }
                                            ?>
                                            @endforeach
                                        ...
                                        <tr class="bg-dark text-white">
                                            ...
                                            <?php 
                                                $oilTypes2 = \App\Models\DataSetting::where('group_type', "oilGroup")->where('parent_id', $oilGroups2['id'])->where('is_deleted', '0')->where('is_active','1')->get(); 
                                            ?>
                                                @foreach ($oilTypes2 as $oilType2)
                                                <?php
                                                     $oilTypesDs2 = \App\Models\DataSetting::where('group_type', "oilGroup")->where('parent_id', $oilType2['id'])->where('is_deleted', '0')->where('is_active','1')->get(); 
                                    ...
                                    @if (!empty($infos))
                                        ...
                                        <tr>
                                            ...
                                            <?php 
                                                $oilTypes3 = \App\Models\DataSetting::where('group_type', "oilGroup")->where('parent_id', $oilGroups3['id'])->where('is_deleted', '0')->where('is_active','1')->get(); 
                                                if(count($oilTypes3) > 0){
                                            ?>
                                                @foreach ($oilTypes3 as $oilType3)
                                                <?php
                                                     $oilTypesDs3 = \App\Models\DataSetting::where('group_type', "oilGroup")->where('parent_id', $oilType3['id'])->where('is_deleted', '0')->where('is_active','1')->get(); 
                                    ...
                                    <tr>
                                        ...
                                        <?php 
                                            $oilTypes4 = \App\Models\DataSetting::where('group_type', "oilGroup")->where('parent_id', $oilGroups4['id'])->where('is_deleted', '0')->where('is_active','1')->get(); 
                                        ...
                                        ?>
                                            ...
                                            <?php
                                                    $oilTypesDs4 = \App\Models\DataSetting::where('group_type', "oilGroup")->where('parent_id', $oilType4['id'])->where('is_deleted', '0')->where('is_active','1')->get(); 

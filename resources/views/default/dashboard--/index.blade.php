...
<script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: [
        <?php if ($meeting_lists && count($meeting_lists) > 0) : ?>
        <?php foreach ($meeting_lists as $meeting_list) : ?>
        <?php 
        $s_date = date('Y-m-d', strtotime($meeting_list->date_start)).'T'.date('H:i:s', strtotime($meeting_list->date_start));
        $e_date = date('Y-m-d', strtotime($meeting_list->date_end)).'T'.date('H:i:s', strtotime($meeting_list->date_end));
        ?>
          {

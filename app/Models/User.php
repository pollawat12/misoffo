...
class User extends Authenticatable
{
    ...
    public static function selcetUserLeaveDepartments($departmentId=0 , $userId=0 )
    {

        $employees = UserDiagram::where('user_id',$userId)->where('is_active',1)->first();

        //select role วนิดา is_approved = 0
 
        //select role ไพลิน is_approved = 1
      ...
      // $isApproved = 99;
        ...
        $query = DB::table('users as t1')
            ...
            ->where($conditions)
            ->whereIn('t3.is_approved', $isApproved )
            ->select('t3.*');
    ...
    public static function getEmployeesName($arrConds=[] , $id = 0 , $isCount=false)
    {
        $conditions = [
            'id' => (int) $id,
            'is_deleted' => (int) 0,

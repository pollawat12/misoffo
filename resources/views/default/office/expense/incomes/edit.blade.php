...
<div class="content">
    ...
    <div class="container-fluid">
        ...
        <form action="{{url('office/expenses/incomes/update')}}" method="POST" name="frm-save" id="frm-save">
            <div class="row">
                <div class="col-md-12">
                    <div class="card-box">
                        <h4 class="header-title mb-4"><i class="mdi mdi-file-document-box-plus" style="font-size: 16px !important; "> แก้ไขข้อมูล รายได้ของเงินกองทุน</i> </h4>
                        ...
                        <h4 class="header-title mb-4">
                            ...
                            <!-- <a href="{{url('office/budgets/institution/all')}}/{{$id}}?yearid={{$yearid}}&budgetsid={{$budgetsid}}&institutionid={{$info->institution_id}}&id={{$id}}" target="_blank"><button type="button" class="btn btn-warning btn-sm"><i class="mdi mdi-file-multiple"> ดูรายการทั้งหมด</i></button></a> -->
                                ...
                                <thead>
                                    ...
                                    <?php $FundDetail = \App\Models\AssetIncomesDetail::where('asset_id', $id)->where('type_id', $item['id'])->where('is_deleted', '0')->where('is_active','1')->first();?>
                                    <tbody>
                                    ...
                                    <?php             
                                        ...
                                        if (!empty($valdetails)){
                                            foreach ($valdetails as $valdetail){
                                                ...
                                                $FundDetail1 = \App\Models\AssetIncomesDetail::where('asset_id', $id)->where('type_id', $valdetail['id'])->where('is_deleted', '0')->where('is_active','1')->first();
                                    ?>
                                            <tbody>
                                                <tr>
                                                    <td class="align-middle">
                                                ...
                                                <?php             
                                                    $valassets = \App\Models\AssetIncomesDetail::where('asset_id', $id)->where('parent_id', $valdetail['id'])->where('is_deleted', '0')->where('is_active','1')->get();
                                                    if (!empty($valdetails)){
                            ...
                            <button type="submit" class="btn btn-primary">
                                    ...
                                    <a href="{{URL('office/expenses/incomes/lists')}}"  class="btn btn-secondary"><i class=" mdi mdi-backspace-outline
                                        "> ยกเลิก</i></a>
                    </div>
        ...
        <!-- end row -->
        <div id="div-data-url" data-url="{{url('office/expenses/incomes/edit')}}/{{$id}}"></div>
        <div id="url-redirect-back" data-url="{{url('office/expenses/incomes/edit')}}/{{$id}}"></div>
    </div>
</div>

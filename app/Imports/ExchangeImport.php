    ...
    */
    public function model(array $row)
    {
        ...
        if (!empty($row[0])) {
            ...
            $process->oil_price_b100 = (!empty($row[1]))  ? trim($row[1]) : 0.00;
            $process->oil_price_peninsula = (!empty($row[2]))  ? trim($row[2]) : 0.00;
            $process->oil_price_sabah = (!empty($row[3]))  ? trim($row[3]) : 0.00;

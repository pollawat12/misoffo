...
class Budget extends Model
{
     ...
     * @return void
     */
    public static function getDataYearAll($id=0 , $parentId=0, $isCount=false)
    {
        if($id != 0){
        ...
        if (!empty($records)) {
            foreach ($records as $row) {
                ...
                $info = BudgetYearDetail::find((int) $row->budget_type);
                ...
                if($info->budget_id == '325'){
                    ...
                    if($row->projects_id != 0){
                        ...
                        $projectDetail = BudgetYearDetail::find((int) $row->projects_id);   

                    }elseif($row->projects_id_1 != 0){

                        $projectDetail = BudgetYearDetail::find((int) $row->projects_id_1); 

                    }else{

                        $projectDetail = BudgetYearDetail::find((int) $row->projects_id_2); 
                ...
                $categroys = BudgetYearDetail::where('id',$row->budget_categroy)->where('is_deleted', '0')->where('is_active','1')->get();
                foreach ($categroys as $categroy);

                $statementtype = DataSetting::getNameDataByValueAndType($categroy['statementtype_id'],'statementtype');

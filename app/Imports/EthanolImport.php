...
class EthanolImport implements ToModel, WithStartRow
{
    //
    protected $id;
    ...
    */
    public function model(array $row)
    {
        ...
        if (!empty($row[0])) {
            ...
            $process->oil_price_ethanol_ref = (!empty($row[1]))  ? trim($row[1]) : 0.00;
            $process->oil_price_lpg_cargo = '0.00';
            $process->oil_price_ethanol_cas = (!empty($row[2]))  ? trim($row[2]) : 0.00;
            $process->oil_price_ethanol_mol = (!empty($row[3]))  ? trim($row[3]) : 0.00;
            $process->oil_price_ethanol_cas_mol = (!empty($row[4]))  ? trim($row[4]) : 0.00;
            $process->oil_price_b100 = 0.00;
            $process->oil_price_peninsula = 0.00;
            $process->oil_price_sabah = 0.00;
            $process->oil_price_type = 12;
            $process->detail = NULL;

        ...
        <div class="form-group">
            ...
            <?php if($typeid == 1):?>
                <select name="certificate_detail_name[]" id="certificate_detail_name" class="form-control" style="height: 45px;">
                    ...
                    <option value="{{$valitems['id']}}">{{$valitems['expense_item']}} / ค่าใช้จ่าย</option>
                    @endforeach
                    @endif
                    @if (count($purchases) > 0)
                    @foreach($purchases as $purchase)
                    <option value="{{$purchase['id']}}">{{$purchase['purchases_order_number']}} / จัดซื้อ - จัดจ้าง</option>
                    @endforeach
                    @endif
                </select>
            <?php else: ?>
                <select name="certificate_detail_name[]" id="certificate_detail_name" class="form-control" style="height: 45px;">
                    ...
                    @if (count($items) > 0)
                    @foreach($items as $valitems)
                    <option value="{{$valitems['id']}}">{{$valitems['compensate_num']}}</option>
                    @endforeach
                    @endif
                </select>
            <?php endif; ?>
...
$(document).ready(function() {
    ...
    $(add_button).click(function(e){ 
        ...
        <?php else: ?>
            var fname_lname_new = '<div class="row"><div class="col-md-6"><div class="form-group"><label for="certificate_detail_name">รายการ <code>*</code> </label><select name="certificate_detail_name[]" id="certificate_detail_name" class="form-control" style="height: 45px;"><option value="">--เลือก--</option><?php if(count($items) > 0){ foreach($items as $valitems){ ?><option value="<?php echo $valitems['id']; ?>"><?php echo $valitems['compensate_num'] ?></option><?php } }?></select></div></div><div class="col-md-5"><div class="form-group"><label for="certificate_detail_note"> หมายเหตุ </label><input type="text" name="certificate_detail_note[]" class="form-control" placeholder="" style="height: 45px;"></div></div><button type="button" class="form-control btn btn-danger btn-sm col-md-1 remove_field" style="height: 45px;margin-block:30px;"> ลบ </button></div>';
        <?php endif; ?>

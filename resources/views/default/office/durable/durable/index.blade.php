        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <h4 class="page-title">แสดง ข้อมูลพัสดุ</h4> 
                </div>
            </div>
        </div>
        <!-- end page title -->
        ...
        <div class="row">
            <div class="col-12">
                <div class="card-box table-responsive">
                    <div class="row mb-3">
                        <div class="col-6">
                            <h4 class="header-title">รายการข้อมูล :: ข้อมูลพัสดุ</h4>
                            <p class="sub-header"></p>
                        </div>

                        <div class="col-6 text-right">
                            <!-- <a href="{{URL('office/durable/activity/search/list')}}/?t=durable&pr=0" target="_blank"><button type="button" class="btn btn-sm btn-dark"><i class="mdi mdi-database-plus"> นำข้อมูลออก</i></button></a> -->
                            <a href="{{URL('office/durable/add')}}/?t=durable&pr=0" class="btn btn-sm btn-primary width-md waves-light"><i class="mdi mdi-database-plus"> เพิ่มข้อมูล</i></a>
                        </div>
                    </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="align-middle" style="width: 9%;">
                                    @if(!empty($item['durable_image']))
                                            <img src="{{ URL($item['durable_image']) }}" class="img-thumbnail" alt="..." style="max-height: 80px !important; width:auto; height: 80px;">
                                    @endif
                                </td>
                                <td class="align-middle" style="width: 15%;">{{$item['durable_name']}}</td>
                                <td class="align-middle" style="width: 12%;">{{$item['durable_serial']}}</td>
                                <td class="align-middle" style="width: 12%;">{{$item['durable_number']}}</td>
                                {{-- <td class="align-middle" style="width: 10%;">{{getDateShow($item['durable_received_date'])}}</td>
                                <td class="align-middle" style="width: 10%;">{{number_format($item['durable_price'],2,'.',',')}}</td> --}}
                                <td class="align-middle" style="width: 6%;">@if ($item['durable_status'] == 0) ในคลัง @elseif($item['durable_status'] == 1) ยืม @else ตัดจำหน่าย @endif</td>
                                <td class="align-middle" style="width: 8%;">
                                    {{-- <a href="{{URL('office/durable/report')}}/{{$item['id']}}/?t=durable&pr=0" class="btn btn-outline-primary waves-effect width-md waves-light btn-sm"><i class="mdi mdi-file-document-box-outline">รายงาน</i> </a>
                                    <a href="{{URL('office/durable/edit')}}/{{$item['id']}}/?t=durable&pr=0" class="btn btn-outline-warning waves-effect width-md waves-light btn-sm"><i class="mdi mdi-pencil-plus-outline">แก้ไข</i> </a>
                                    <a href="{{URL('office/durable/delete')}}/{{$item['id']}}" class="btn btn-outline-danger waves-effect width-md waves-light btn-sm" ><i class="mdi mdi-trash-can-outline">ลบ</i></a> --}}

                                    <select name="input_action" class="form-control select-new" data-id="{{$item['id']}}">
                                        <option value="">เลือก</option>
                                        <option value="report" data-original-title="">รายงาน</option>
                                        <!-- <option value="downloads" data-original-title="{{$item['durable_invoice_file']}}">invoice</option> -->

...
{    
    ...
    public function index(Request $request)
    {
        ...
        if($pr == 1){
            ...
            if($CheckDubai > 0){ $SumDubai =  '(+'.number_format($CheckDubai, 4, '.', '').')';}else{ $SumDubai =  '('.number_format($CheckDubai, 4, '.', '').')';  }
            if($CheckBrent > 0){ $SumBrent =  '(+'.number_format($CheckBrent, 4, '.', '').')';}else{ $SumBrent =  '('.number_format($CheckBrent, 4, '.', '').')';  }
            if($CheckWTI > 0){ $SumWTI =  '(+'.number_format($CheckWTI, 4, '.', '').')';}else{ $SumWTI =  '('.number_format($CheckWTI, 4, '.', '').')';  }
            ...
            if($CheckDubaiMax > 0){ $SumDubaiMax =  '(+'.number_format($CheckDubaiMax, 4, '.', '').')';}else{ $SumDubaiMax =  '('.number_format($CheckDubaiMax, 4, '.', '').')';  }
            if(($CheckBrentMax) > 0){ $SumBrentMax =  '(+'.number_format($CheckBrentMax, 4, '.', '').')';}else{ $SumBrentMax =  '('.number_format($CheckBrentMax, 4, '.', '').')';  }
            if(($CheckWTIMax) > 0){ $SumWTIMax =  '(+'.number_format($CheckWTIMax, 4, '.', '').')';}else{ $SumWTIMax =  '('.number_format($CheckWTIMax, 4, '.', '').')';  }
            // end crude
            ...
            if($CheckUnl95 > 0){ $SumUnl95 =  '(+'.number_format($CheckUnl95, 4, '.', '').')';}else{ $SumUnl95 =  '('.number_format($CheckUnl95, 4, '.', '').')';  }
            if($CheckUnl91 > 0){ $SumUnl91 =  '(+'.number_format($CheckUnl91, 4, '.', '').')';}else{ $SumUnl91 =  '('.number_format($CheckUnl91, 4, '.', '').')';  }
            if($CheckGasoil005 > 0){ $SumGasoil005 =  '(+'.number_format($CheckGasoil005, 4, '.', '').')';}else{ $SumGasoil005 =  '('.number_format($CheckGasoil005, 4, '.', '').')';  }
            if($CheckGasoil001 > 0){ $SumGasoil001 =  '(+'.number_format($CheckGasoil001, 4, '.', '').')';}else{ $SumGasoil001 =  '('.number_format($CheckGasoil001, 4, '.', '').')';  }
            ...
            if($CheckUnl95Max > 0){ $SumUnl95Max =  '(+'.number_format($CheckUnl95Max, 4, '.', '').')';}else{ $SumUnl95Max =  '('.number_format($CheckUnl95Max, 4, '.', '').')';  }
            if($CheckUnl91Max > 0){ $SumUnl91Max =  '(+'.number_format($CheckUnl91Max, 4, '.', '').')';}else{ $SumUnl91Max =  '('.number_format($CheckUnl91Max, 4, '.', '').')';  }
            if($CheckGasoil005Max > 0){ $SumGasoil005Max =  '(+'.number_format($CheckGasoil005Max, 4, '.', '').')';}else{ $SumGasoil005Max =  '('.number_format($CheckGasoil005Max, 4, '.', '').')';  }
            if($CheckGasoil001Max > 0){ $SumGasoil001Max =  '(+'.number_format($CheckGasoil001Max, 4, '.', '').')';}else{ $SumGasoil001Max =  '('.number_format($CheckGasoil001Max, 4, '.', '').')';  }
            // enc Inside  
            ...
            $checkLPG = $LGP - $LGPTo;
            if($checkLPG > 0){ $SumLPG =  '(+'.number_format($checkLPG, 4, '.', '').')';}else{ $SumLPG =  '('.number_format($checkLPG, 4, '.', '').')';  }
            ...
            if($checlkExchange > 0){ $SumExchange =  '(+'.number_format($checlkExchange, 4, '.', '').')';}else{ $SumExchange =  '('.number_format($checlkExchange, 4, '.', '').')';  }


            // end Exchange
            $price = [
                ['label' => 'Dubai','s_price' => $DubaiTo,'e_price' => $Dubai,'diff_price' => $SumDubai],
                ['label' => 'Brent','s_price' => $BrentTo,'e_price' => $Brent,'diff_price' => $SumBrent],
                ['label' => 'WTI','s_price' => $WTITo,'e_price' => $WTI,'diff_price' => $SumWTI],
                ['label' => 'Unl 95','s_price' => $Unl95To,'e_price' => $Unl95,'diff_price' => $SumUnl95],
                ['label' => 'Unl 91 (Non-Oxy)','s_price' => $Unl91To,'e_price' => $Unl91,'diff_price' => $SumUnl91],
                ['label' => 'Gasoil 0.05','s_price' => $Gasoil005To,'e_price' => $Gasoil005,'diff_price' => $SumGasoil005],
                ['label' => 'Gasoil 0.00001','s_price' => $Gasoil001To,'e_price' => $Gasoil001,'diff_price' => $SumGasoil001],
                ['label' => 'LPG','s_price' => $LGPTo,'e_price' => $LGP,'diff_price' => $SumLPG],
                ['label' => 'Exchange','s_price' => $ExchangeTo,'e_price' => $Exchange,'diff_price' => $SumExchange]
            ]; 

            $price_end = [
                ['label' => 'Dubai','s_price' => $DubaiToMax,'e_price' => $DubaiMax,'diff_price' => $SumDubaiMax],
                ['label' => 'Brent','s_price' => $BrentToMax,'e_price' => '','diff_price' => ''],
                ['label' => 'WTI','s_price' => $WTIToMax,'e_price' => '','diff_price' => ''],
                ['label' => 'Unl 95','s_price' => $Unl95MaxTo,'e_price' => $Unl95Max,'diff_price' => $SumUnl95Max],
                ['label' => 'Unl 91 (Non-Oxy)','s_price' => $Unl91MaxTo,'e_price' => $Unl91Max,'diff_price' => $SumUnl91Max],
                ['label' => 'Gasoil 0.05','s_price' => $Gasoil005MaxTo,'e_price' => $Gasoil005Max,'diff_price' => $SumGasoil005Max],
                ['label' => 'Gasoil 0.00001','s_price' => $Gasoil001MaxTo,'e_price' => $Gasoil001Max,'diff_price' => $SumGasoil001Max],
                ['label' => 'LPG','s_price' => $LGPTo,'e_price' => $LGP,'diff_price' => $SumLPG],
                ['label' => 'Exchange','s_price' => $ExchangeTo,'e_price' => $Exchange,'diff_price' => $SumExchange]
            ];
            ...
            if (count($infoLPGs) > 0){
                ...
                //             $infoLPG1 = $infoLPGD['oil_min'];
                //         }elseif($infoLPGD['oil_type'] == '469'){

                //             $infoLPG2 = $infoLPGD['oil_min'];
                //         }elseif($infoLPGD['oil_type'] == '470'){

                //             $infoLPG3 = $infoLPGD['oil_min'];
                //         }elseif($infoLPGD['oil_type'] == '471'){

                //             $infoLPG4 = $infoLPGD['oil_min'];
                //         }
